...
Func _SteamCMDCreate($tForceReset = False)
	...
	Else
		Local $tLogin = $aSteamCMDUserName & " " & $aSteamCMDPassword
	EndIf
	...
	If $aServerVer <> "public" Then
		$tTxtValY = StringReplace($tTxtValY, "+quit", "-beta " & $aServerVer & " ^" & @CRLF & "+quit")
		$tTxtValN = StringReplace($tTxtValN, "+quit", "-beta " & $aServerVer & " ^" & @CRLF & "+quit")
	EndIf
...
Func _SteamCMDBatchFilesCreate()
	...
	FileDelete($aBatchDIR & "\Launch_" & $aGameName & ".bat")
	If $aServerExtraCMD <> "" Then
		Local $tExtraCMD = $aServerExtraCMD & " "
	Else
		Local $tExtraCMD = ""
	EndIf
	Local $tServerStart = """" & $aServerDirLocal & "\" & $aServerEXE & """ " & $aLaunchParams & " " & $tExtraCMD & $aConfigFile
	FileWrite($aBatchDIR & "\Launch_" & $aGameName & ".bat", "Start """ & $aUtilName & """ " & $tServerStart & @CRLF & "Exit")
...
Func _StartRemoteRestart()
	If $aRemoteRestartUse = "yes" Then
		...
		If $MainSocket = -1 Then
			...
			LogWrite(" [RmRrst] Remote Restart enabled but could not bind to " & $aServerIP & ":" & $aRemoteRestartPort)
		Else
			If $sObfuscatePass = "no" Then
				LogWrite(" [RmRrst] Remote Restart enabled. Listening for restart request at http://" & $aServerIP & ":" & $aRemoteRestartPort & "/?[key]=[password]", " [RmRrst] Remote Restart enabled. Listening for restart request at http://" & $aServerIP & ":" & $aRemoteRestartPort & "/?" & $aRemoteRestartKey & "=" & $aRemoteRestartCode)
			Else
				LogWrite(" [RmRrst] Remote Restart enabled. Listening for restart request at http://" & $aServerIP & ":" & $aRemoteRestartPort & "/?[key]=[password]")
			EndIf
		EndIf
	EndIf
...
Func _ErrFunc($oError = 0)
	...
	LogWrite(" [ Util ] [" & $aObjErrFunc & "] Error in ObjEvent [0x" & Hex($oError.number) & "] " & _ErrorCode($tHex))
EndFunc   ;==>_ErrFunc
Func _CloseEAH()
...
;~ 	Local $tCmd = '"' & $aFolderTemp & '7z" ' & $aBackupCommandLine & ' -xr0!"' & $aGameName & '_Backup*.zip" "' & $tFull & '" "' & _
;~ 			$tConfigPath & '" "' & $aServerDataFolder & '" "' & $aServerSaveGame & '" '
	Local $tCmd = '"' & $aFolderTemp & '7z" ' & $aBackupCommandLine & ' -xr0!"' & $aGameName & '_Backup*.zip" "' & $tFull & '" "' & _
			$tConfigPath & '" "' & $aServerSaveGame & '" '
...
Func PurgeBackups()
	...
	LogWrite("", " [Backup] Deleting Backups > " & $aBackupNumberToKeep & " in folder " & $aBackupOutputFolder)
	Run($aPurgeBackups, "", @SW_HIDE)
EndFunc   ;==>PurgeBackups
Func _GetPlayerCount()
	$aOnlinePlayers = GetPlayerCount(False)
	If $aPlayerCountErr Then
;~ 	If $aGameTime = "Day 1, 00:00" Then
...
;~ 		If $aGameTime = "Day 1, 00:00" Then
			...
			Sleep(1000)
			$aOnlinePlayers = GetPlayerCount(False)
			If $aPlayerCountErr Then LogWrite("", " [Player] Failed to get player count.")
		...
		For $i = 1 To 5
			...
			Else
				...
				If $aServerOnlinePlayerYN = "no" Then
					...
					If Number($tQueryCheckResult[6]) <> $tPlayers Then
						If $aPlayerCountErr Then
			...
			If $i = 5 Then
				If $tRestart1 Then
					...
					If $tFailedCountQuery > $aWatchdogAttemptsBeforeRestart Then
						...
						Local $x1Return = _GetEAHDefaultConfigFile()
						If $x1Return[0] <> $aConfigFile Then LogWrite(" [Config] !!! NOTE !!! EAH is set to use config file [" & $x1Return[0] & "].  ESUU is set to use config file [" & $aConfigFile & "] PLEASE UPDATE ESUU's CONFIG TO USE DESIRED YAML CONFIG.")
						If $aServerCrashMsgSentRestartTF = False And $sUseDiscordBotServerCrashMsgRestartYN <> "no" Then _SendDiscordCrash($sDiscordServerCrashMsgRestart)
						$aServerCrashMsgSentRestartTF = True
						$tFailedCountQuery = 0
						CloseServer($aTelnetIP, $aTelnetPort, $aTelnetPass)
...
Func _SendDiscordChat($tMsg)
	If $sUseDiscordBotPlayerChatYN = "yes" Then _SendDiscordMsg(StringReplace($tMsg, "[Global] ", ""), $aServerDiscordWHSelChat)
...
Func _GetServerNameFromLog($tSplash = 0)
	Local $tReturn = ""
	Local $sLogPath = IniRead($aUtilCFGFile, "CFG", "Last Log Time Stamp", $aServerDirLocal & '\' & $aGameName & 'Server_Data\output_log_dedi' & StringRegExpReplace(_NowCalc(), "[\\\/\: ]", "_") & ".txt")
	Local $sLogPathOpen = FileOpen($sLogPath, 0)
...
Func CloseServer($ip, $port, $pass, $tNoSplashOff = "no")
	...
	If $aRebootConfigUpdate = "no" Then
		If $tNoSplashOff = "no" Then $aSplash = _Splash("Shutting down " & $aGameName & " server . . .", 0, 350, 110)
	EndIf
	...
	For $i = 1 To 5
		...
		$aReply = _PlinkSend($aTelnetShutdown)
		LogWrite(" [Server] Sending shutdown command [" & $aTelnetShutdown & "] to server. Countdown:" & (6 - $i) & " Response [" & StringStripCR($aReply) & "]")
		If StringInStr($aReply, "Exit will be executed") = 0 Then
			Sleep(1000)
		Else
			If $aTelnetStayConnectedYN = "no" Then _PlinkDisconnect()
			ExitLoop
	...
	For $i = 1 To 10
		If ProcessExists($aServerPID) Then
			...
			LogWrite(" [Server] (PID: " & $aServerPID & ") Server failed to shutdown. Killing process. Countdown:" & (11 - $i))
			Sleep(1000)
		Else
			ExitLoop
		EndIf
...
Func _Discord_ErrFunc($oError)
	LogWrite(" [Server] (PID: " & $aServerPID & ") Error: 0x" & Hex($oError.number) & " While Sending Discord Bot Message.")
...
Func TwitchMsgLog($sT_Msg)
	...
	If $aTwitchIRC[0] Then
		LogWrite(" [Server] (PID: " & $aServerPID & ") [Twitch Bot] Successfully Connected to Twitch IRC")
		If $aTwitchIRC[1] Then
			LogWrite(" [Server] (PID: " & $aServerPID & ") [Twitch Bot] Username and OAuth Accepted. [" & $aTwitchIRC[2] & "]")
			If $aTwitchIRC[3] Then
				LogWrite(" [Server] (PID: " & $aServerPID & ") [Twitch Bot] Successfully sent ( " & $sT_Msg & " ) to all Channels")
			Else
				LogWrite(" [Server] (PID: " & $aServerPID & ") [Twitch Bot] ERROR | Failed sending message ( " & $sT_Msg & " ) to one or more channels")
			EndIf
		Else
			LogWrite(" [Server] (PID: " & $aServerPID & ") [Twitch Bot] ERROR | Username and OAuth Denied [" & $aTwitchIRC[2] & "]")
		EndIf
	Else
		LogWrite(" [Server] (PID: " & $aServerPID & ") [Twitch Bot] ERROR | Could not connect to Twitch IRC. Is this URL or port blocked? [irc.chat.twitch.tv:6667]")
	EndIf
...
Func RunExternalScriptDaily()
	If $aExternalScriptDailyYN = "yes" Then
		LogWrite(" [Batch ] Executing DAILY restart external script " & $aExternalScriptDailyFile)
...
Func RunExternalScriptAnnounce()
	If $aExternalScriptAnnounceYN = "yes" Then
		LogWrite(" [Batch ] Executing FIRST ANNOUNCEMENT external script " & $aExternalScriptAnnounceFile)
		If $aExternalScriptAnnounceWait = "no" Then
			If $aExternalScriptHideYN = "yes" Then
...
Func RunExternalRemoteRestart()
	If $aExternalScriptRemoteYN = "yes" Then
		LogWrite(" [Batch ] Executing REMOTE RESTART external script " & $aExternalScriptRemoteFile)
...
Func RunExternalScriptUpdate()
	If $aExternalScriptUpdateYN = "yes" Then
		LogWrite(" [Batch ] Executing Script When Restarting For Server Update: " & $aExternalScriptUpdateFile)
	...
	If $aExecuteExternalScript = "yes" Then
		...
		If $sFileExists = 0 Then
			...
			Else
				...
				LogWrite(" [Batch ] External BEFORE update script execution disabled - Could not find " & $aExternalScriptFile)
			EndIf
		EndIf
	EndIf
	If $aExternalScriptValidateYN = "yes" Then
		...
		If $sFileExists = 0 Then
			...
			Else
				...
				LogWrite(" [Batch ] External AFTER update script execution disabled - Could not find " & $aExternalScriptValidateFile)
			EndIf
		EndIf
	EndIf
	If $aExternalScriptDailyYN = "yes" Then
		...
		If $sFileExists = 0 Then
			...
			Else
				...
				LogWrite(" [Batch ] External DAILY restart script execution disabled - Could not find " & $aExternalScriptDailyFile)
			EndIf
		EndIf
	EndIf
	If $aExternalScriptUpdateYN = "yes" Then
		...
		If $sFileExists = 0 Then
			...
			Else
				...
				LogWrite(" [Batch ] External UPDATE restart script execution disabled - Could not find " & $aExternalScriptUpdateFile)
			EndIf
		EndIf
	EndIf
	If $aExternalScriptAnnounceYN = "yes" Then
		...
		If $sFileExists = 0 Then
			...
			Else
				$aExternalScriptDailyYN = "no"
				LogWrite(" [Batch ] External DAILY restart script execution disabled - Could not find " & $aExternalScriptAnnounceFile)
			EndIf
		EndIf
	EndIf
	If $aExternalScriptRemoteYN = "yes" Then
		...
		If $sFileExists = 0 Then
			...
			Else
				$aExternalScriptDailyYN = "no"
				LogWrite(" [Batch ] External DAILY restart script execution disabled - Could not find " & $aExternalScriptRemoteFile)
			EndIf
		EndIf
	EndIf
...
Func GetLatestVerSteamDB($bSteamAppID, $bServerVer)
	...
	If $hBuildID < 100000 Then
		...
		LogWrite(" [Update] Error retrieving buildid via SteamDB website. Please visit:" & $aURL & _
...
;~ Func GetLatestVersionOld($sCmdDir)
;~ 	$hBuildID = "0"
;~ 	Local $aReturn[2] = [False, ""]
...
Func UpdateCheck($tAsk, $tSplash = 0, $tShowIfNoUpdate = False)
	...
	Else
		If $aFirstBoot Or $tAsk Then
			If $tSplash > 0 Then
				ControlSetText($tSplash, "", "Static1", $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Acquiring latest BuildID.")
			Else
				$aSplash = _Splash($aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Acquiring latest BuildID.")
			EndIf
		EndIf
...
Func SteamUpdate()
	...
	If ($sManifestExists = 1) And ($aFirstBoot = 0) Then
		...
		LogWrite("", " [Update] Notice: Install manifest found at " & $aSteamCMDDir & "\steamapps\appmanifest_" & $aSteamAppID & ".acf & renamed to appmanifest_" & $aSteamAppID & "_" & $TimeStamp & ".acf")
	Else
		$aFirstBoot = 0
	EndIf
	If ($sManifestExists = 1) And ($aFirstBoot = 0) Then
		...
		LogWrite("", " [Update] Notice: Install manifest found at " & $aServerDirLocal & "\steamapps\appmanifest_" & $aSteamAppID & ".acf & renamed to appmanifest_" & $aSteamAppID & "_" & $TimeStamp & ".acf")
	Else
		$aFirstBoot = 0
	EndIf
	$aSteamUpdateNow = False
	If $aValidate = "yes" Then
		LogWrite(" [Update] Running SteamCMD " & $aServerVer & " branch with validate.", " [Steam Update] Running SteamCMD " & $aServerVer & " branch with validate: ['" & $aSteamUpdateCMDValY & "']")
		RunWait('"' & $aSteamUpdateCMDValY & '"')
	Else
		LogWrite(" [Update] Running SteamCMD " & $aServerVer & " branch without validate.", " [Steam Update] Running SteamCMD " & $aServerVer & " branch without validate. ['" & $aSteamUpdateCMDValN & "']")
		...
		Else
			...
			If $tMB = 6 Then
				...
				Else
					...
					If $tMB = 1 Then
						LogWrite(" [ Util ] Util Shutdown - Initiated by User to run update.")
						If $aRemoteRestartUse = "yes" Then
							TCPShutdown()
						EndIf
						IniWrite($aUtilCFGFile, "CFG", "PID", $aServerPID)
...
Func _ShowLoginLogo()
	...
	Else
		...
		_GDIPlus_Shutdown()
		$hGUI_LoginLogo = GUICreate($aGameName & " Logo", $iW, $iH, -1, -1, $WS_POPUP, $WS_EX_LAYERED)
...
Func _RemoteRestart($vMSocket, $sCodes, $sKey, $sHideCodes = "no", $sServIP = "127.0.0.1", $sName = "" & $aGameName & " Server")
...
Func ReadUini($aIniFile, $sLogFile)
	...
	Global $aServerVer = IniRead($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Version (public/experimental) ###", $iniCheck)
	...
	Global $aEAH_Use = IniRead($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Start and stop EmpyrionAdminHelper (EAH) with Server? (Needed for some Steam updates) (Always/Updates/Never) ###", $iniCheck)
...
;~ 	Global $aAppendVerShort = IniRead($aIniFile, " --------------- APPEND SERVER VERSION TO NAME --------------- ", "Use SHORT name (B9) or LONG (Alpha 17.1 (B9))? (short/long) ###", $iniCheck)
	...
	Global $aWatchdogWaitServerUpdate = IniRead($aIniFile, " --------------- KEEP ALIVE WATCHDOG ---------------", "Pause watchdog for _ minutes after server updated to allow for update processes to complete (1-360) ###", $iniCheck)
	...
	Global $aBackupAddedFolders = IniRead($aIniFile, " --------------- BACKUP --------------- ", "Additional backup folders / files (comma separated. Folders add \ at end. ex. C:\" & $aGameName & "\,D:\" & $aGameName & " Server\) ###", $iniCheck)
...
;~ 	Global $sUseDiscordBotPlayerDiedYN = IniRead($aIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message when player dies? (yes/no) ###", $iniCheck)
	...
	Global $aPlayerSeparator = IniRead($aIniFile, " --------------- DISCORD MESSAGES --------------- ", "Announcement Online Player separator (Use ; for [space]) ###", $iniCheck)
...
;~ 	Global $sDiscordPlayerDiedMsg = IniRead($aIniFile, " --------------- DISCORD MESSAGES --------------- ", "Player Died Message (\p - Player Name, \n Next Line) ###", $iniCheck)
	...
	Global $aEAH_Exe = IniRead($aIniFile, " --------------- " & StringUpper($aUtilName) & " MISC OPTIONS --------------- ", "EAH exe filename ###", $iniCheck)
	Global $aEAH_Dir = IniRead($aIniFile, " --------------- " & StringUpper($aUtilName) & " MISC OPTIONS --------------- ", "EAH exe folder ###", $iniCheck)
...
;~ 	Global $aFPRenameModsYN = IniRead($aIniFile, " --------------- (ALMOST) FUTURE PROOF UPDATE OPTIONS --------------- ", "Rename the Mod Folder (therefore saving and disabling it) if Future Proof was needed (3 consecutive failed starts after an update)? (yes/no) ###", $iniCheck)
	...
	If $iniCheck = $aServerDirLocal Then
		$aServerDirLocal = @ScriptDir & "\" & $aGameName & " Dedicated Server"
		$iIniFail += 1
...
;~ 	EndIf
	If $iniCheck = $aServerIP Then
		...
		$iIniError = $iIniError & "AppendVerShort, "
	EndIf
	If $iniCheck = $aEAH_Use Then
		$aEAH_Use = "Always"
		$iIniFail += 1
		$iIniError = $iIniError & "EAH_Use, "
	EndIf
	...
	ElseIf $aQueryCheckSec < 30 Then
		$aQueryCheckSec = 30
		LogWrite(" [Config] Query Port server-is-alive check interval was out of range. Interval set to: " & $aQueryCheckSec & " seconds.")
	...
	ElseIf $aTelnetCheckSec < 30 Then
		$aTelnetCheckSec = 30
		LogWrite(" [Config] Telnet server-is-alive check interval was out of range. Interval set to: " & $aTelnetCheckSec & " seconds.")
	ElseIf $aTelnetCheckSec > 900 Then
		$aTelnetCheckSec = 900
		LogWrite(" [Config] Telnet server-is-alive check interval was out of range. Interval set to: " & $aTelnetCheckSec & " seconds.")
	EndIf
	If $iniCheck = $aWatchdogWaitServerUpdate Then
		$aWatchdogWaitServerUpdate = "5"
		$iIniFail += 1
		$iIniError = $iIniError & "TelnetDelayAfterUpdate, "
	ElseIf $aWatchdogWaitServerUpdate < 1 Then
		$aWatchdogWaitServerUpdate = 1
		LogWrite(" [Config] Telnet server-is-alive check interval was out of range. Interval set to: " & $aWatchdogWaitServerUpdate & " minutes.")
	...
	ElseIf $aWatchdogWaitServerStart < 1 Then
		...
		LogWrite(" [Config] Watchdog wait for server to start was out of range. Interval set to: " & $aWatchdogWaitServerStart & " minutes.")
	...
	ElseIf $aWatchdogAttemptsBeforeRestart < 1 Then
		...
		LogWrite(" [Config] Watchdog Attempts Before Restart was out of range. Attempts set to: " & $aWatchdogAttemptsBeforeRestart & ".")
	...
	ElseIf $aUpdateCheckInterval < 5 Then
		...
		LogWrite(" [Config] Update check interval was out of range. Interval set to: " & $aUpdateCheckInterval & " minutes.")
	EndIf
	...
	If $iniCheck = $sInGameUpdateMessage Then
		$sInGameUpdateMessage = $aGameCreators & " has released a new update. Server is rebooting in \m minute(s)."
		$iIniFail += 1
	...
	If $iniCheck = $aServerDiscord1BotName Then
		$aServerDiscord1BotName = "" & $aGameName & " Bot"
		$iIniFail += 1
		$iIniError = $iIniError & "ServerDiscord1BotName, "
	EndIf
	If $iniCheck = $aServerDiscord1Avatar Then
		$aServerDiscord1Avatar = "http://www.phoenix125.com/share/Discord/DiscordAvatarEmpyrion.jpg"
		$iIniFail += 1
	...
	If $iniCheck = $aServerDiscord2BotName Then
		$aServerDiscord2BotName = "" & $aGameName & " Bot"
		$iIniFail += 1
		$iIniError = $iIniError & "ServerDiscord2BotName, "
	EndIf
	If $iniCheck = $aServerDiscord2Avatar Then
		$aServerDiscord2Avatar = "http://www.phoenix125.com/share/Discord/DiscordAvatarEmpyrion.jpg"
		$iIniFail += 1
	...
	If $iniCheck = $aServerDiscord3BotName Then
		$aServerDiscord3BotName = "" & $aGameName & " Bot"
		$iIniFail += 1
		$iIniError = $iIniError & "ServerDiscord3BotName, "
	EndIf
	If $iniCheck = $aServerDiscord3Avatar Then
		$aServerDiscord3Avatar = "http://www.phoenix125.com/share/Discord/DiscordAvatarEmpyrion.jpg"
		$iIniFail += 1
	...
	If $iniCheck = $aServerDiscord4BotName Then
		$aServerDiscord4BotName = "" & $aGameName & " Bot"
		$iIniFail += 1
		$iIniError = $iIniError & "ServerDiscord4BotName, "
	EndIf
	If $iniCheck = $aServerDiscord4Avatar Then
		$aServerDiscord4Avatar = "http://www.phoenix125.com/share/Discord/DiscordAvatarEmpyrion.jpg"
		$iIniFail += 1
	...
	If $iniCheck = $sDiscordUpdateMessage Then
		$sDiscordUpdateMessage = $aGameCreators & " has released a new update. Server is rebooting in \m minute(s)."
		$iIniFail += 1
	...
	If $iniCheck = $sDiscordServersUpMessage Then
		$sDiscordServersUpMessage = $aGameName & " server is online and ready for connection."
		$iIniFail += 1
	...
	If $iniCheck = $sTwitchUpdateMessage Then
		$sTwitchUpdateMessage = $aGameCreators & " has released a new update. Server is rebooting in \m minute(s)."
		$iIniFail += 1
...
;~ 	EndIf
	...
	If $iniCheck = $sDiscordPlayersMsg Then
		$sDiscordPlayersMsg = 'Players Online: **\o / \m**   :rocket:  \a\n\j\l'
		$iIniFail += 1
...
;~ 	EndIf
	...
	EndIf
	If $iniCheck = $aEAH_Exe Then
		$aEAH_Exe = "EmpAdminHelper.exe"
		$iIniFail += 1
		$iIniError = $iIniError & "EAH_Exe, "
	EndIf
...
;~ 	EndIf
	...
	LogWrite(" [Config] Importing settings from " & $aGameName & "ServerUtil.ini.")
	...
	If $aEAH_Dir = "Temp" Then
		$aEAH_Dir = $aServerDirLocal & "\DedicatedServer\EmpyrionAdminHelper"
		IniWrite($aIniFile, " --------------- " & StringUpper($aUtilName) & " MISC OPTIONS --------------- ", "EAH exe folder ###", $aEAH_Dir)
	EndIf
	If $iniCheck = $aEAH_Dir Then
		$aEAH_Dir = $aServerDirLocal & "\DedicatedServer\EmpyrionAdminHelper"
		$iIniFail += 1
		$iIniError = $iIniError & "EAH_Dir, "
	EndIf
...
Func _UpdateAnnouncements()
	...
	If $sUseDiscordBotRemoteRestart = "yes" Or $sUseDiscordBotDaily = "yes" Or $sUseDiscordBotUpdate = "yes" Or $sUseTwitchBotRemoteRestart = "yes" Or $sUseTwitchBotDaily = "yes" Or _
			$sUseTwitchBotUpdate = "yes" Or $sInGameAnnounce = "yes" Then
		...
		Global $aDailyMsgInGame = AnnounceReplaceTime($tAnnounceNotifyTime1, $sInGameDailyMessage)
		Global $aDailyMsgDiscord = AnnounceReplaceTime($tAnnounceNotifyTime1, $sDiscordDailyMessage)
		Global $aDailyMsgTwitch = AnnounceReplaceTime($tAnnounceNotifyTime1, $sTwitchDailyMessage)
		Global $aDailyTime = StringSplit($tAnnounceNotifyTime1, ",")
		Global $aDailyCnt = Int($aDailyTime[0])
		Global $aUpdateMsgInGame = AnnounceReplaceTime($tAnnounceNotifyTime2, $sInGameUpdateMessage)
		Global $aUpdateMsgDiscord = AnnounceReplaceTime($tAnnounceNotifyTime2, $sDiscordUpdateMessage)
		Global $aUpdateMsgTwitch = AnnounceReplaceTime($tAnnounceNotifyTime2, $sTwitchUpdateMessage)
		Global $aUpdateTime = StringSplit($tAnnounceNotifyTime2, ",")
		Global $aUpdateCnt = Int($aUpdateTime[0])
		Global $aRemoteMsgInGame = AnnounceReplaceTime($tAnnounceNotifyTime3, $sInGameRemoteRestartMessage)
		Global $aRemoteMsgDiscord = AnnounceReplaceTime($tAnnounceNotifyTime3, $sDiscordRemoteRestartMessage)
		Global $aRemoteMsgTwitch = AnnounceReplaceTime($tAnnounceNotifyTime3, $sTwitchRemoteRestartMessage)
		Global $aRemoteTime = StringSplit($tAnnounceNotifyTime3, ",")
...
Func AddTrailingSlash($aString)
	If StringRight($aString, 1) <> "\" Then $aString &= "\"
	Return $aString
EndFunc   ;==>AddTrailingSlash
Func _FindLatestFileRecursive($dir, $mask)
	Local $tFolder = _FindLatestFolder($dir, "*.*")
	Return _FindLatestFile($tFolder, $mask)
EndFunc   ;==>_FindLatestFileRecursive
Func _FindLatestFile($dir, $mask)
	Local $latestTime = 0, $currentTime = 0
	Local $files = _FileListToArray($dir, $mask, 1)
	If @error = 1 Then Return False
	If @error = 4 Then Return False
	...
	Next
	Return $latestFile
EndFunc   ;==>_FindLatestFile
Func _FindLatestFolder($dir, $mask)
	Local $latestTime = 0, $currentTime = 0
	Local $files = _FileListToArray($dir, $mask, 2)
	If @error = 1 Then Return False
	If @error = 4 Then Return False
	For $i = 1 To UBound($files) - 1
		$currentFile = $dir & $files[$i]
...
Func iniFileCheck($aIniFile, $iIniFail, $iIniError)
	...
	If $tFileLen > 8000 Then
		...
		LogWrite(" [Config] INI MISMATCH: Found " & $iIniFail & " missing variable(s) in " & $aUtilName & ".ini. Backup created and all existing settings transfered to new INI. Please modify INI and restart.")
		LogWrite(" [Config] INI MISMATCH: Parameters missing: " & $iIniFail)
		_SplashOff()
...
Func UpdateIni($aIniFile)
	...
	IniWrite($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Version (public/experimental) ###", $aServerVer)
...
;~ 	IniWrite($aIniFile, " --------------- APPEND SERVER VERSION TO NAME --------------- ", "Use SHORT name (B9) or LONG (Alpha 17.1 (B9))? (short/long) ###", $aAppendVerShort)
	...
	IniWrite($aIniFile, " --------------- BACKUP --------------- ", "Additional backup folders / files (comma separated. Folders add \ at end. ex. C:\" & $aGameName & "\,D:\" & $aGameName & " Server\) ###", $aBackupAddedFolders)
...
;~ 	IniWrite($aIniFile, " --------------- DISCORD INTEGRATION --------------- ", "Send Discord message when player dies? (yes/no) ###", $sUseDiscordBotPlayerDiedYN)
	...
	IniWrite($aIniFile, " --------------- DISCORD MESSAGES --------------- ", "Announcement Online Player separator (Use ; for [space]) ###", $aPlayerSeparator)
...
;~ 	Global $aConfigFileTemp = "ServerConfig" & $aServerName & "ServerUtilTemp.xml"
;~ 	Global $aConfigFileTempFull = $aServerDirLocal & "\" & $aConfigFileTemp
;~ 	Local $tConfigPath = $aServerDirLocal & "\" & $aConfigFile
;~ 	FileDelete($aConfigFileTempFull)
;~ 	Local $tConfigPathOpen = FileOpen($tConfigPath, 0)
;~ 	Local $tConfigRead2 = FileRead($tConfigPathOpen)
;~ 	Local $tConfigRead1 = StringRegExpReplace($tConfigRead2, "</ServerSettings>", "<!-- BEGIN EmpyriontSEerverUtility Changes -->" & @CRLF)
;~ 	FileWrite($aConfigFileTempFull, $tConfigRead1)
...
Func _CheckForExistingServer()
	Local $tPID = _CheckIfProcessRunning($aServerEXE, $aServerDirLocal)
	If $tPID > 0 Then
		IniWrite($aUtilCFGFile, "CFG", "PID", $tPID)
		LogWrite(" [Server] Existing server found by Process search. PID(" & $tPID & ")")
	EndIf
	Return $tPID
EndFunc   ;==>_CheckForExistingServer
Func _CheckForExistingPlink()
	Local $xFileName = _PathSplitEx($aFilePlink)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tReturn2 = 0
	...
	For $x = 1 To $tProcess[0][0]
		...
		If $tProcessFolder = $aFilePlink Then
			...
			If $tReturn2 <> $aPlinkPID Then
				IniWrite($aUtilCFGFile, "CFG", "Plink_PID", $tReturn2)
...
Func PIDReadServer($tSplash = 0)
	...
	Else
		...
		If ProcessExists($tReturn) Then
			...
			If $tSplash = 0 Then
				$aSplash = _Splash($aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Existing server found." & @CRLF & "PID(" & $tReturn & ")")
			Else
				ControlSetText($tSplash, "", "Static1", $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Existing server found." & @CRLF & "PID(" & $tReturn & ")")
			EndIf
		Else
			$aNoExistingPID = True
...
Func _CheckForExistingEAH($tSplash = 0)
	Local $tPID = _CheckIfProcessRunning($aEAH_Exe, $aEAH_Dir)
	If $tPID > 0 Then
		$aEAH_PID = $tPID
		LogWrite(" [ EAH  ] Existing " & $aEAH_Name & " process found. PID(" & $aEAH_PID & ")")
		IniWrite($aUtilCFGFile, "CFG", "EAH_PID", $aEAH_PID)
		If $tSplash > 0 Then
			ControlSetText($tSplash, "", "Static1", $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Existing EAH found." & @CRLF & "PID(" & $aEAH_PID & ")")
			Sleep(2500)
		EndIf
	Else
	EndIf
	Return $aEAH_PID
EndFunc   ;==>_CheckForExistingEAH
Func TrayUtilConfig()
	GUI_Config()
...
EndFunc   ;==>TrayUtilLog
Func TrayServerLog()
	Local $tLatestDedicatedLogFile = _FindLatestFileRecursive($aServerDirLocal & "\Logs\", "Dedicated*.log")
;~ 	Local $sLogPath = IniRead($aUtilCFGFile, "CFG", "Last Log Time Stamp", $aServerDirLocal & '\' & $aGameName & 'Server_Data\output_log_dedi' & StringRegExpReplace(_NowCalc(), "[\\\/\: ]", "_") & ".txt")
	ShellExecute($tLatestDedicatedLogFile)
...
Func TrayExitCloseY()
	LogWrite(" [Server] Utility exit with server shutdown initiated by user via tray icon (Exit: Shut Down Servers).")
...
Func TrayRemoteRestart()
	LogWrite(" [RmRrst] Remote Restart requested by user via tray icon (Initiate Remote Restart).")
	If $aRemoteRestartUse <> "yes" Then
		...
		If $tMB = 6 Then ; (YES)
			LogWrite(" [RmRrst] Remote Restart enabled in " & $aUtilName & ".ini per user request")
			...
			If $MainSocket = -1 Then
				...
				LogWrite(" [RmRrst] Remote Restart enabled. Could not bind to " & $aServerIP & ":" & $aRemoteRestartPort)
;~ 				_ExitUtil()
			Else
				If $sObfuscatePass = "no" Then
					LogWrite("", " [RmRrst] Remote Restart enabled. Listening for restart request at http://" & $aServerIP & ":" & $aRemoteRestartPort & "/?" & $aRemoteRestartKey & "=" & $aRemoteRestartCode)
				Else
					LogWrite(" [RmRrst] Remote Restart enabled. Listening for restart request at http://" & $aServerIP & ":" & $aRemoteRestartPort & "/?[key]=[password]")
				EndIf
			EndIf
		Else
			LogWrite(" [RmRrst] No changes made to Remote Restart setting in " & $aUtilName & ".ini per user request.")
	...
	Else
		...
		If $tMB = 6 Then ; (YES)
			If $aBeginDelayedShutdown = 0 Then
				LogWrite(" [RmRrst] Remote Restart request initiated by user.")
				If ($sUseDiscordBotDaily = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotDaily = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($sInGameAnnounce = "yes") Then
					$aRebootReason = "remoterestart"
					$aBeginDelayedShutdown = 1
					$aTimeCheck0 = _NowCalc
			...
			EndIf
		Else
			LogWrite(" [RmRrst] Remote Restart request canceled by user.")
...
Func TraySendMessage()
	...
	Else
		If $aTelnetSayQuoteOrApostrophe = "quote" Then
			$tMsg = "say """ & $tMsg & """"
		Else
			$tMsg = "say '" & $tMsg & "'"
		EndIf
...
Func _ImportServerConfig()
	Local $tReturn = False
	Local $tChangesTF = False
	Local $tTxt = ""
	Local $tFound = ""
	Local $tFileReadArray = FileReadToArray($sConfigPath)
	For $i = 0 To UBound($tFileReadArray) - 2
;~ 		ControlSetText($tSplash, "", "Static1", $aStartText & "Importing settings from " & $aConfigFile & "." & @CRLF & "Analyzing line: " & $i)
;~ 		$tFileReadArray[$i] = StringReplace($tFileReadArray[$i], ": ", ":")
		Local $tSplit = StringSplit($tFileReadArray[$i], ":")
		If StringInStr($tFileReadArray[$i], "Srv_Port:") Then
			$tFound &= "Srv_Port,"
			If StringInStr($tFileReadArray[$i], "# Srv_Port:") Then
				$tChangesTF = True
				$tFileReadArray[$i] = StringReplace($tFileReadArray[$i], "# Srv_Port:", "Srv_Port:")
				$tTxt &= "Error! Server Port was disabled and is now enabled in " & $aConfigFile & @CRLF
			EndIf
			If $tSplit[0] > 1 Then
				$tSplit[2] = StringTrimLeft($tSplit[2], 1)
				$aServerPort = Number($tSplit[2])
			Else
				$aServerPort = 0
				$tTxt &= "Error importing Server Port from " & $aConfigFile & @CRLF
			EndIf
			LogWrite(" [Config] . . . Imported Srv_Port:" & $aServerPort)
		EndIf
		If StringInStr($tFileReadArray[$i], "Srv_Name:") Then
			$tFound &= "Srv_Name,"
			If StringInStr($tFileReadArray[$i], "# Srv_Name:") Then
				$tChangesTF = True
				$tFileReadArray[$i] = StringReplace($tFileReadArray[$i], "# Srv_Name:", "Srv_Name:")
				$tTxt &= "Error! Server Name was disabled and is now enabled in " & $aConfigFile & @CRLF
			EndIf
			If $tSplit[0] > 1 Then
				$tSplit[2] = StringTrimLeft($tSplit[2], 1)
				$aServerName = $tSplit[2]
			Else
				$aServerName = "My Empyrion Server"
		...
		If StringInStr($tFileReadArray[$i], "Tel_Enabled:") Then
			...
			If StringInStr($tFileReadArray[$i], "# Tel_Enabled:") Then
				$tChangesTF = True
				$tFileReadArray[$i] = "    Tel_Enabled: true   "
				$tTxt &= "Error! Telnet was disabled and is now enabled in " & $aConfigFile & @CRLF
			EndIf
		EndIf
		If StringInStr($tFileReadArray[$i], "Tel_Port:") Then
			$tFound &= "Tel_Port,"
			If StringInStr($tFileReadArray[$i], "# Tel_Port:") Then
				$tChangesTF = True
				$tFileReadArray[$i] = StringReplace($tFileReadArray[$i], "# Tel_Port:", "Tel_Port:")
				$tTxt &= "Error! Telnet Port was disabled and is now enabled in " & $aConfigFile & @CRLF
			EndIf
			If $tSplit[0] > 1 Then
				$tSplit[2] = StringTrimLeft($tSplit[2], 1)
				$aTelnetPort = Number($tSplit[2])
			Else
				$aTelnetPort = 0
				$tTxt &= "| Error importing Telnet Port from " & $aConfigFile & @CRLF
			EndIf
			LogWrite(" [Config] . . . Imported Tel_Port:" & $aTelnetPort)
		EndIf
		If StringInStr($tFileReadArray[$i], "Tel_Pwd:") Then
			$tFound &= "Tel_Pwd,"
			If StringInStr($tFileReadArray[$i], "# Tel_Pwd:") Then
				$tChangesTF = True
				$tFileReadArray[$i] = StringReplace($tFileReadArray[$i], "# Tel_Pwd:", "Tel_Pwd:")
				$tTxt &= "Error! Telnet Password was disabled and is now enabled in " & $aConfigFile & @CRLF
			EndIf
			If $tSplit[0] > 1 Then
				$tSplit[2] = StringTrimLeft($tSplit[2], 1)
				$aTelnetPass = $tSplit[2]
			Else
				$aTelnetPass = "ESUUabc123"
				$tTxt &= "Error importing Telnet Port from " & $aConfigFile & @CRLF
			EndIf
			If $sObfuscatePass = "no" Then
				LogWrite(" [Config] . . . Imported Tel_Pwd:" & $aTelnetPass)
			Else
				LogWrite(" [Config] . . . Imported Tel_Pwd:{hidden}")
			EndIf
		EndIf
	Next
	If StringInStr($tFound, "Tel_Pwd") = 0 Then
		$aTelnetPass = "ESUUabc123"
		_ArrayAdd($tFileReadArray, "Tel_Pwd:" & $aTelnetPass)
		If $sObfuscatePass = "no" Then
			LogWrite(" [Config] No Tel_Pwd found in config. New line added: Tel_Pwd:" & $aTelnetPass)
		Else
			LogWrite(" [Config] No Tel_Pwd found in config. New line added: Tel_Pwd:{hidden}")
		EndIf
		$tChangesTF = True
	EndIf
	If StringInStr($tFound, "Tel_Port") = 0 Then
		$aTelnetPass = "ESUUabc123"
		_ArrayAdd($tFileReadArray, "Tel_Port:" & $aTelnetPass)
		If $sObfuscatePass = "no" Then
			LogWrite(" [Config] No Tel_Pwd found in config. New line added: Tel_Pwd:" & $aTelnetPass)
		Else
			LogWrite(" [Config] No Tel_Pwd found in config. New line added: Tel_Pwd:{hidden}")
		...
		$tChangesTF = True
	EndIf
	;Or StringInStr($tFound, "Tel_Port") = 0 Or StringInStr($tFound, "Tel_Enabled") = 0 Then
	If $tChangesTF Then
		Local $xFileName = _PathSplitEx($sConfigPath)
		Local $tFileName = $xFileName[3] & $xFileName[4]
		Local $tFolder = $xFileName[1] & $xFileName[2]
		Local $tTime = @YEAR & "-" & @MON & "-" & @MDAY & "_" & @HOUR & "-" & @MIN
		Local $tFileSave = $xFileName[3] & "_" & $tTime & "_bak" & $xFileName[4]
		If FileExists($sConfigPath) Then FileMove($sConfigPath, $tFolder & $tFileSave)
		_FileWriteFromArray($sConfigPath, $tFileReadArray)
		LogWrite(" [Config] " & StringReplace($tTxt, @CRLF, " | "))
		$tReturn = True
	EndIf
	Return $tReturn
;~ 	Srv_Port:
...
;~ 		Else
;~ 			LogWrite(" . . . Server telnet password was " & $aTelnetPass & ". Password CHANGED to: " & $aUtilName & ". Recommend change telnet password in " & $aConfigFile)
;~ 		EndIf
;~ 		Global $aTelnetPass = $aUtilName
;~ 		$aServerTelnetReboot = "yes"
...
Func GetPlayerCount($tSplash)
	...
	TraySetIcon(@ScriptName, 201)
	If $tSplash Then $aSplash = _Splash("Checking online players. . .")
	Global $xPlayersList = TelnetOnlinePlayers($aTelnetIP, $aTelnetPort, $aTelnetPass)
	If $xPlayersList[0] < 0 Then
	...
	Else
		$tOnlinePlayers[0] = False
		$tOnlinePlayers[1] = "Total Players " ; Screen version with @CRLF
		$tOnlinePlayers[2] = "Total Players " ; Log version without @CRLF
		If $xPlayersList[0] = 0 Then
			$aServerPlayers = "0"
...
;~ 			$aPlayersOnlineSteamID = ""
		Else
			Global $tUserCnt = UBound($xPlayersList) - 1
...
;~ 				$tOnlinePlayers[2] = $tOnlinePlayers[2] & $tUserAll[$i] & " [" & $tSteamIDArray[$i] & "] , "
		...
		EndIf
		If $aRCONError Then
			LogWrite(" [Player] Error receiving online players.")
			$aErr = True
			$aRCONError = False
		EndIf
		_SplashOff()
		...
		If ($tPlayersBeforeString <> $aPlayersOnlineName) Then
			...
			$aPlayersJoined = _ArrayToString($tTempArray[1], StringReplace($aPlayerSeparator, ";", " "))
			$aPlayersLeft = _ArrayToString($tTempArray[0], StringReplace($aPlayerSeparator, ";", " "))
			$aPlayersName = _ArrayToString($tPlayersAfterArray, StringReplace($aPlayerSeparator, ";", " "))
			$tOnlinePlayers[0] = True
			LogWrite(" [Player] " & _PlayersChangedText() & $tOnlinePlayers[2])
		...
		Else
			...
			EndIf
		EndIf
		If $aPlayerCountErr = False Then $aOnlinePlayerLast = $tOnlinePlayers[1]
	...
	If IsArray($tArray3) Then
		For $t1 = 1 To ($tArray3[0] - 1)
			If StringInStr($tArray3[$t1], "Players connected (") Then
				$sReturn[0] = Number(_ArrayToString(_StringBetween($tArray3[$t1], "(", ")")))
				ExitLoop
			EndIf
		Next
	EndIf
...
Func _PlinkConnect($tIP, $tPort, $tPwd, $tLog012 = 2, $tSkipVerifyTF = False) ; $tLog0123: 0 = no log, 1 = Detailed log only, 2 = Both logs
	...
	Else
		...
		If Not $aPlinkPID Then
			LogWrite(" [Telnet] ERROR! Failed to connect to telnet. [" & $tIP & ":" & $tPort & "]", " [Telnet] ERROR! Failed to connect to telnet. [" & $tIP & ":" & $tPort & "] " & $tCmd)
...
;~ 		If $tSkipVerifyTF = False Then
		For $i = 1 To 6
			If $i = 6 Then
				Local $x1Return = _GetEAHDefaultConfigFile()
				If $x1Return[0] <> $aConfigFile Then
					...
					LogWrite("          NOTE!!! EAH is set to use config file [" & $x1Return[0] & "].  ESUU is set to use config file [" & $aConfigFile & "] PLEASE UPDATE ESUU's CONFIG TO USE DESIRED YAML CONFIG.")
				Else
					LogWrite(" [Telnet] ERROR! Plink failed to connect to server [" & $tIP & ":" & $tPort & "] PID:" & $aPlinkPID, " [Telnet] ERROR! Plink failed to connect to server [" & $tIP & ":" & $tPort & "]  PID[" & $aPlinkPID & "]" & $tCmd)
				EndIf
			...
			If StringInStr($kReturn, "Logged in successfully") > 0 Then
				LogWrite(" [Telnet] Plink logged in successfully", " [Telnet] Plink logged in successfully [" & ReplaceCRLF($kReturn) & "]")
				ExitLoop
			EndIf
			Sleep(500)
		Next
...
Func _GetEAHDefaultConfigFile()
	Local $tEAH_YAML = $aServerDirLocal & "\DedicatedServer\EmpyrionAdminHelper\Config\Settings.XML"
	Local $sReturn[3]
	If FileExists($tEAH_YAML) Then
		Local $tRead = FileRead($tEAH_YAML)
		Local $tParam = "Dedicated"
		Local $tArray = _StringBetween($tRead, "<" & $tParam & ">", "</" & $tParam & ">")
		$sReturn[0] = $tArray[0]
		Local $tParam = "Server_IP"
		Local $tArray = _StringBetween($tRead, "<" & $tParam & ">", "</" & $tParam & ">")
		$sReturn[1] = $tArray[0]
		Local $tParam = "Telnet_Port"
		Local $tArray = _StringBetween($tRead, "<" & $tParam & ">", "</" & $tParam & ">")
		$sReturn[2] = $tArray[0]
		Return $sReturn
	Else
		...
		Return $sReturn
	EndIf
EndFunc   ;==>_GetEAHDefaultConfigFile
...
;~ 	If StringRegExp($tTxt4, "Player '.*' joined the game") Then
	If StringInStr($tTxt4, "Got player id") Then
		Local $tName = _ArrayToString(_StringBetween($tTxt4, "'", "'"))
...
Func _TelnetLookForLeave($tTxt4)
	If StringInStr($tTxt4, "' disconnected") Then
		Local $tName = _ArrayToString(_StringBetween($tTxt4, "'", "'"))
		Local $tMsg4 = $sDiscordPlayerLeftMsg
		$tMsg4 = StringReplace($tMsg4, "\p", $tName)
		$tMsg4 = StringReplace($tMsg4, "\n", @CRLF)
		_SendDiscordLeave($tMsg4)
		_PlayersOnlineCheck()
	EndIf
EndFunc   ;==>_TelnetLookForLeave
Func _TelnetLookForChat($tTxt4)
	If StringInStr($tTxt4, "CHAT Player") Or StringInStr($tTxt4, "CHAT ServerForward") Then
		If StringInStr($tTxt4, "CHAT Player") Then
			Local $tID = _ArrayToString(_StringBetween($tTxt4, " from ", "/Faction"))
			Local $tName = _GetPlayerNameFromID($tID)
			Local $tType = _ArrayToString(_StringBetween($tTxt4, "CHAT Player/", " from " & $tID))
		EndIf
		If StringInStr($tTxt4, "CHAT ServerForward") Then
			Local $tName = "Admin"
			Local $tType = "Global"
		EndIf
		Local $tChat = _ArrayToString(_StringBetween($tTxt4 & "!Q", ": '", "'!Q"))
...
Func _GetPlayerNameFromID($tID)
	For $i = 0 To (UBound($xPlayerID) - 1)
		If $xPlayerID[$i] = $tID Then Return $xPlayerName[$i]
	Next
	Sleep(1000)
	_GetPlayerNamesIDsFromLog($tID)
	For $i = 0 To (UBound($xPlayerID) - 1)
		If $xPlayerID[$i] = $tID Then Return $xPlayerName[$i]
	Next
	Return "Unknown"
EndFunc   ;==>_GetPlayerNameFromID
Func _GetPlayerNamesIDsFromLog($tID)
	Local $tLatestDedicatedLogFile = _FindLatestFileRecursive($aServerDirLocal & "\Logs\", "Dedicated*.log")
	Local $tReadArray = FileReadToArray($tLatestDedicatedLogFile)
	Global $xPlayerID[1]
	Global $xPlayerName[1]
	...
	For $i = 0 To (UBound($tReadArray) - 1)
		...
		EndIf
	Next
EndFunc   ;==>_GetPlayerNamesIDsFromLog
...
;~ 		GUISetIcon("G:\Game Server Files\AutoIT\Icons\phoenix.ico", -1)
		...
		Global $W1_T1_R_BranchExperimental = GUICtrlCreateRadio("experimental", 191, 221, 98, 19)
		GUICtrlSetFont(-1, 10, 400, 0, "MS Sans Serif")
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		GUICtrlSetOnEvent(-1, "W1_T1_R_BranchExperimentalClick")
		Global $W1_T1_I_SteamBranch = GUICtrlCreateInput("W1_T1_I_SteamBranch", 315, 218, 156, 22)
		GUICtrlSetFont(-1, 8, 400, 0, "arial")
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		Global $W1_T1_R_BranchManual = GUICtrlCreateRadio("", 299, 221, 15, 17)
		GUICtrlSetFont(-1, 10, 400, 0, "MS Sans Serif")
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		Global $W1_T1_I_BackupCmd = GUICtrlCreateInput("W1_T1_I_DIR", 596, 531, 284, 22)
		GUICtrlSetFont(-1, 8, 400, 0, "arial")
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		Global $Label80 = GUICtrlCreateLabel("Additional backup folders / files (comma separated. Folders add \ at end. ex. ""C:\Empyrion\"",""D:\Empyrion\*.yaml"")", 24, 535, 568, 17)
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		GUICtrlCreateGroup("", -99, -99, 1, 1)
		Global $Group8 = GUICtrlCreateGroup("Empyrion Admin Helper", 615, 191, 273, 173)
		GUICtrlSetFont(-1, 10, 400, 0, "Arial")
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		Global $Pic1 = GUICtrlCreatePic($aFolderTemp & "eah-logo.jpg", 792, 218, 89, 123)
		GUICtrlSetOnEvent(-1, "Pic1Click")
		Global $Label93 = GUICtrlCreateLabel("Start/Close EAH with Server", 620, 222, 167, 20)
		GUICtrlSetOnEvent(-1, "Label93Click")
		Global $W1_T1_R_EAH_Always = GUICtrlCreateRadio("Always", 627, 243, 113, 17)
		GUICtrlSetOnEvent(-1, "W1_T1_R_EAH_AlwaysClick")
		Global $W1_T1_R_EAH_Updates = GUICtrlCreateRadio("Updates Only *", 627, 265, 115, 17)
		GUICtrlSetOnEvent(-1, "W1_T1_R_EAH_UpdatesClick")
		Global $W1_T1_R_EAH_Never = GUICtrlCreateRadio("Never", 627, 286, 71, 17)
		GUICtrlSetOnEvent(-1, "W1_T1_R_EAH_NeverClick")
		Global $Label94 = GUICtrlCreateLabel("* Recommended:", 622, 314, 85, 18)
		GUICtrlSetFont(-1, 8, 400, 0, "Arial")
		GUICtrlSetOnEvent(-1, "Label94Click")
		Global $Label95 = GUICtrlCreateLabel("Updates may fail if EAH running", 624, 332, 156, 18)
		GUICtrlSetFont(-1, 8, 400, 0, "Arial")
		...
		GUICtrlCreateGroup("", -99, -99, 1, 1)
		Global $Pic2 = GUICtrlCreatePic($aFolderTemp & "Drone.jpg", 610, 359, 222, 180)
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		GUICtrlCreateGroup("", -99, -99, 1, 1)
		Global $Pic4 = GUICtrlCreatePic($aFolderTemp & "Large_Constructor.jpg", 570, 406, 97, 109)
		GUICtrlSetOnEvent(-1, "Pic4Click")
		Global $Pic8 = GUICtrlCreatePic($aFolderTemp & "100px-Drill.jpg", 132, 440, 77, 77)
		GUICtrlSetOnEvent(-1, "Pic8Click")
		...
		Global $Label103 = GUICtrlCreateLabel("Online Player Separator (Use ; for [space])", 23, 383, 202, 17, $SS_RIGHT)
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		GUICtrlSetOnEvent(-1, "Label103Click")
		GUICtrlCreateGroup("", -99, -99, 1, 1)
		Global $Label26 = GUICtrlCreateLabel("\o Online Player Count, \m Max Players, \j Joined Sub-Msg, \l Left Sub-Msg, \a Online Players Sub-Msg, \n Next Line", 38, 253, 626, 18)
		GUICtrlSetFont(-1, 8, 800, 0, "Arial")
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		GUICtrlCreateGroup("", -99, -99, 1, 1)
		Global $Pic5 = GUICtrlCreatePic($aFolderTemp & "99px-Oxygen_Bottle.jpg", 718, 316, 95, 117)
		GUICtrlSetOnEvent(-1, "Pic5Click")
		Global $Pic9 = GUICtrlCreatePic($aFolderTemp & "Pumpkin_Sprout.jpg", 724, 126, 81, 81)
		GUICtrlSetOnEvent(-1, "Pic9Click")
		Global $Tab10 = GUICtrlCreateTabItem("9 FINISH / SAVE")
		Global $Pic7 = GUICtrlCreatePic($aFolderTemp & "empylogo1.jpg", 236, 335, 429, 197)
		GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
		...
		Else
			If $tNewInstallTF Then
				...
				MsgBox(4096, $aUtilName, "Welcome to " & $aUtilName & "! Please modify desired values then choose RESTART Util in the last tab." & @CRLF & @CRLF & "Visit us on Discord: http://discord.gg/EU7pzPs", 60)
			EndIf
...
Func _UpdateWindowConfig()
	...
	ElseIf $aServerVer = "experimental" Then
	...
	If $aEAH_Use = "Always" Then
		GUICtrlSetState($W1_T1_R_EAH_Always, $GUI_CHECKED)
	ElseIf $aEAH_Use = "Updates" Then
		GUICtrlSetState($W1_T1_R_EAH_Updates, $GUI_CHECKED)
	Else
		GUICtrlSetState($W1_T1_R_EAH_Never, $GUI_CHECKED)
	EndIf
...
;~ 	GUICtrlSetData($W1_T6_I_PlayerDie, $sDiscordPlayerDiedMsg)
	...
	GUICtrlSetData($W1_T6_I_OnlinePlayerSeparator, $aPlayerSeparator)
...
;~ 	_ImportServerConfig()
;~ 	ConfigClose()
	_Splash($aGameName & " Config Import Updated.", 750)
;~ 	GUI_Config()
...
Func W1_T1_R_EAH_AlwaysClick()
	$aEAH_Use = "Always"
	IniWrite($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Start and stop EmpyrionAdminHelper (EAH) with Server? (Needed for some Steam updates) (Always/Updates/Never) ###", $aEAH_Use)
EndFunc   ;==>W1_T1_R_EAH_AlwaysClick
Func W1_T1_R_EAH_NeverClick()
	$aEAH_Use = "Never"
	IniWrite($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Start and stop EmpyrionAdminHelper (EAH) with Server? (Needed for some Steam updates) (Always/Updates/Never) ###", $aEAH_Use)
EndFunc   ;==>W1_T1_R_EAH_NeverClick
Func W1_T1_R_EAH_UpdatesClick()
	$aEAH_Use = "Updates"
	IniWrite($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Start and stop EmpyrionAdminHelper (EAH) with Server? (Needed for some Steam updates) (Always/Updates/Never) ###", $aEAH_Use)
EndFunc   ;==>W1_T1_R_EAH_UpdatesClick
...
Func _SortString($tString1, $tOpt = "ND") ; $tOpt N=IsNumber, D=Remove Duplicates
	Local $tString = $tString1
	...
	If StringLeft($tString, 2) = "0," Then $tString = StringTrimLeft($tString, 2)
	If $tString <> $tString1 Then SetError(1)
	Return $tString
EndFunc   ;==>_SortString
...
Func W1_T6_I_OnlinePlayerSeparatorChange()
	$aPlayerSeparator = GUICtrlRead($W1_T6_I_OnlinePlayerSeparator)
	IniWrite($aIniFile, " --------------- DISCORD MESSAGES --------------- ", "Announcement Online Player separator (Use ; for [space]) ###", $aPlayerSeparator)
EndFunc   ;==>W1_T6_I_OnlinePlayerSeparatorChange
...
Func W1_T8_B_ExecuteAFirstRestartAnnouncementClick()
	...
	Local $xFileName = _PathSplitEx($aExternalScriptAnnounceFile)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tFolder = $xFileName[1] & $xFileName[2]
...
Func W1_T8_B_ExecuteAfterRestartClick()
	...
	Local $xFileName = _PathSplitEx($aExternalScriptDailyFile)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tFolder = $xFileName[1] & $xFileName[2]
	If $tFolder = "" Then $tFolder = @ScriptDir
	Local $tInput = FileOpenDialog("Please select file", @ScriptDir, "All (*.*)", 3, $tFileName)
...
Func W1_T8_B_ExecuteBeforeSteamandStart()
	...
	Local $xFileName = _PathSplitEx($aExternalScriptFile)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tFolder = $xFileName[1] & $xFileName[2]
	If $tFolder = "" Then $tFolder = @ScriptDir
	Local $tInput = FileOpenDialog("Please select file", @ScriptDir, "All (*.*)", 3, $tFileName)
...
Func W1_T8_B_ExecuteAfterSteamBeforeStartClick()
	...
	Local $xFileName = _PathSplitEx($aExternalScriptValidateFile)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tFolder = $xFileName[1] & $xFileName[2]
	If $tFolder = "" Then $tFolder = @ScriptDir
	Local $tInput = FileOpenDialog("Please select file", @ScriptDir, "All (*.*)", 3, $tFileName)
...
Func W1_T8_B_ExecuteAfterUpdateClick()
	...
	Local $xFileName = _PathSplitEx($aExternalScriptUpdateFile)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tFolder = $xFileName[1] & $xFileName[2]
	If $tFolder = "" Then $tFolder = @ScriptDir
	Local $tInput = FileOpenDialog("Please select file", @ScriptDir, "All (*.*)", 3, $tFileName)
...
Func W1_T8_B_ExecuteRemoteRestartClick()
	...
	Local $xFileName = _PathSplitEx($aExternalScriptRemoteFile)
	Local $tFileName = $xFileName[3] & $xFileName[4]
	Local $tFolder = $xFileName[1] & $xFileName[2]
	If $tFolder = "" Then $tFolder = @ScriptDir
	Local $tInput = FileOpenDialog("Please select file", @ScriptDir, "All (*.*)", 3, $tFileName)
...
;~ 		SetStatusBusy("Restart Serverserver. Waiting for User Input.", "Waiting for User Input")
		...
		Global $W2_RestartServer = GUICreate($aUtilName, 906, 555, -1, -1, BitOR($GUI_SS_DEFAULT_GUI, $WS_SIZEBOX, $WS_THICKFRAME))
		GUISetIcon($aIconFile, 99)
...
Func AddZero($tString)
	...
	Return $tString
EndFunc   ;==>AddZero
Func _CheckIfProcessRunning($s1PID, $s1Path = "") ; $sPId can by PID or process name.  If process name and no $sPath given, returns PID of first process .  If process name and $sPath given, return PID of process running that has same name and folder
	If StringRight($s1Path, 1) <> "\" Then $s1Path &= "\"
	If IsString($s1PID) Then
		Local $s1PList = ProcessList($s1PID)
		If $s1PList[0][0] > 0 Then
			If $s1Path <> "\" Then
				If $s1PList[0][0] > 0 Then
					For $s1x = 1 To $s1PList[0][0]
						If _ProcessGetPathFromPID($s1PList[$s1x][1], 2) = $s1Path Then Return $s1PList[$s1x][1]
					Next
				EndIf
			Else
				Return $s1PList[1][1]
		...
		EndIf
	Else
		If ProcessExists($s1PID) Then
			If $s1Path <> "\" Then
				If _ProcessGetPathFromPID($s1PID, 2) = $s1Path Then Return $s1PID
			Else
				Return $s1PID
			EndIf
		EndIf
	EndIf
	Return 0
EndFunc   ;==>_CheckIfProcessRunning
Func _ProcessGetPathFromPID($vProcessID, $bFullPath = 1) ; $bFullPath 0 = filename/process name only, $bFullPath 1 = full path and filename/process name, 2 = path only (no filename / process name)
	...
	$hProcess = 0
	If $bFullPath = 2 Then
		$stPath = _PathSplitEx($sFilename)
		$sFilename = $stPath[1] & $stPath[2]
	EndIf
	Return SetError(0, 0, $sFilename)
EndFunc   ;==>_ProcessGetPathFromPID
Func _ProcessGetPIDFromName($sProcess)
	If IsString($sProcess) = 0 Then
		SetError(2)
		Return 0
	ElseIf ProcessExists($sProcess) = 0 Then
		SetError(1)
		Return 0
	Else
		Local $sPList = ProcessList($sProcess)
		Local $si
		Local $sPId[$sPList[0][0] + 1]
		$sPId[0] = $sPList[0][0]
		For $si = 1 To $sPList[0][0]
			$sPId[$si] = $sPList[$si][1]
		Next
		If UBound($sPId) > 1 Then
			Return $sPId[1]
		Else
			Return 0
		EndIf
	EndIf
EndFunc   ;==>_ProcessGetPIDFromName
Func _PathSplitEx($sFilePath, $sDrive = "", $sDir = "", $sFilename = "", $sExtension = "")
	If IsArray($sFilePath) Then $sFilePath = StringReplace(_ArrayToString($sFilePath), "|", "")
